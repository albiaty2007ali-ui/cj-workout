...
// اسم شبه مطابق جدًا (≥0.93) يكفي وحده — تقريبًا نفس النص حرفيًا (خطأ إملائي/تكرار حقيقي).
// عتبة مرتفعة عمدًا بعد اختبار حقيقي ضد بيانات إنتاج حقيقية اكتشف False Positive: وصفتين
// عربيتين مختلفتين فعليًا (سالسا الفاصوليا السوداء / كساديا الفاصوليا السوداء) تشتركان بعبارة
// مكوّن طويلة ("الفاصوليا السوداء") فوصلت نسبة التشابه لـ89% رغم إنهما طبقين مختلفين تمامًا —
// عتبة 0.85 كانت تلتقط هذا خطأً. الاسم القريب المعقول + تطابق مكونات كبير معًا (البند أدناه)
// يبقى خط الدفاع الأساسي لحالات زي "Chicken Bowl"/"Chicken Bowl Healthy" (نسبة اسم ~0.75 بس).
const NAME_SIMILARITY_REJECT = 0.93;
